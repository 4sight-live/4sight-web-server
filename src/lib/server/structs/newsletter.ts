...
export namespace Newsletter {
    ...
    MailingList.on('create', async (target) => {
        ...
        if (unsub.isErr()) {
            terminal.error(`Failed to create unsubscribe link for mailing list ${target.data.email}: ${unsub.error}`);
            return;
        }
        Email.send({
    ...
    TeamOnboarding.on('create', async (target) => {
        ...
        if (unsub.isErr()) {
            terminal.error(`Failed to create unsubscribe link for team onboarding ${target.data.email}: ${unsub.error}`);
            return;
        }
