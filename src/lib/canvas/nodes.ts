...
export const render = (canvas: HTMLCanvasElement) => {
    ...
    for (let i = 0; i < NODE_COUNT; i++) {
        const possible = [...Array(NODE_COUNT).keys()]
            .filter(j =>
                j !== i &&
                !adjacency.get(i)!.has(j) &&
                adjacency.get(j)!.size < 3
            );
    ...
    const lineMaterial = new LineMaterial({
        color: 0x00ffff,
        linewidth: .5, // World units — tweak as needed
        transparent: true,
        opacity: 0.8,
        dashed: false,
    ...
    const animate = () => {
        ...
        for (let i = activePulses.length - 1; i >= 0; i--) {
            ...
            (mesh.material as THREE.MeshStandardMaterial).emissiveIntensity = NODE_GLOW_INTENSITY + extraGlow;
        }
